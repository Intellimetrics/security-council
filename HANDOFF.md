...
python3 -m pytest tests/ -q        # 377 green + 1 skip (~1.2s); .venv/bin/python runs all 378 incl. MCP handshake
...
8. **The decision store is target-local and TAMPER-EVIDENT, not tamper-proof** (`<target>/.security-council/decisions/`). R9 (2026-08-24, `docs/reviews/R9-decision-store-trust.md`) found and closed a **live CI-gate bypass** — a hand-written `baseline/latest.json` built from root causes published in the SARIF flipped exit 1 → exit 0 under `gate_baseline: new` (the `ci`/`gov` profiles). Now hardened: G9 (crypto/critical can never be excused by baseline or long-leash suppression; 30-day cap on high-assurance replay), baseline content digest (mismatched **or missing** ⇒ baseline refused ⇒ everything gates), shadow counter cross-checked against real run dirs, malformed records degrade instead of crashing, and per-suppression provenance in the report. **Residual (honest):** an attacker with write access can still recompute the digest and forge an ordinary suppression/baseline entry — the `ssh-keygen -Y` signing lane (design settled by R9 Q1–Q6) is the next milestone. Teams sharing decisions must un-ignore `decisions/` + `baseline/` in *their* repo (run outputs stay ignored) **and put those paths behind CODEOWNERS + required review** — without protected paths, signing would be theater too.
