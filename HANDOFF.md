...
(R9 design)~~ DONE, ~~deterministic verify-fix (re-run scanners on the patched
tree)~~ DONE (below), M-V3 reframe-or-drop, ADO/GitLab on real infrastructure,
`codex-security login` (operator-interactive).
...
**Not functional in 0.1.0, labelled so in `--help`:** `--fix`, `--analyze`.
`--verify-fix` is functional but depends on `--fix`; use `--verify-patch`.
`codex-security` dedicated arm needs an interactive `codex-security login`.
The four R11 fence defects are fixed and live-verified even though the fix
lane is disabled.
