"""Scan orchestration: fan out arms, normalize, cluster, score, report."""
...
def _git_info(target: Path) -> dict:
    r = proc.run_command(["git", "-C", str(target), "rev-parse", "HEAD"], timeout=15)
    if not r.ok:
        return {"git_commit": None, "dirty": None, "branch": None}
    commit = r.stdout.strip()
    st = proc.run_command(["git", "-C", str(target), "status", "--porcelain"], timeout=15)
    br = proc.run_command(["git", "-C", str(target), "rev-parse", "--abbrev-ref", "HEAD"], timeout=15)
    return {"git_commit": commit, "dirty": bool(st.stdout.strip()),
            "branch": br.stdout.strip() if br.ok else None}


def _safe_run(arm: Arm, target: Path, out_dir: Path, run_id: str, collected_at: str) -> ArmResult:
    try:
        return arm.run(target, out_dir, run_id=run_id, collected_at=collected_at)
...
def run_scan(target: str | Path, arms: list[Arm], config: dict, *, out_dir: Path | None = None,
    ...
    with ThreadPoolExecutor(max_workers=max(1, maxc)) as ex:
        results = list(ex.map(
            lambda a: _safe_run(a, target, out_dir, run_id, collected_at), arms))
    ...
    (reports_dir / "raw.sarif").write_text(dumps(sarif.raw_sarif(by_source, tool_version=__version__)))
    (reports_dir / "findings.json").write_text(dumps([to_dict(f) for f in merged]))
    ...
    manifest = build_manifest(
        ...
        started_at=collected_at, finished_at=finished_at, git=_git_info(target),
        degradations=degradations,
        reports=[{"path": str(reports_dir / n), "format": fmt} for n, fmt in
                 (("merged.sarif", "sarif"), ("raw.sarif", "sarif"), ("findings.json", "json"))])
    (reports_dir / "manifest.json").write_text(dumps(manifest))
