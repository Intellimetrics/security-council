...
| `--fix` | fix lane | **Not functional in 0.2.0 — refuses before running anything.** *When* it runs, by design it sends **the scanned tree** to a vendor CLI to generate a patch; that egress is the feature, and this table omitted it (R11). The patch is never applied to your code. |
